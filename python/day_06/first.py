...
until it creates another lanternfish, while another might have 4. So, you can model each fish as a single number that represents the number of days until it creates a new lanternfish.
...
the fifth fish, which has an internal timer of 2. 
...
def reproduce(initial_fishes: np.ndarray, days: int) -> np.ndarray:
    ...
    fishes = np.zeros(9, dtype=int)  # this array will keep count of the number of fishes with each countdown value
...
if __name__ == "__main__":
    ...
    print(reproduce(initial_lanternfishes, 80))
