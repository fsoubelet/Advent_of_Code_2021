...
It's covered in Christmas lights (because of course it is), and it even has an experimental antenna that should be able to track the keys if 
you can boost its signal strength high enough; there's a little meter that indicates the antenna's signal strength by displaying 0-50 stars.
...
The first order of business is to figure out how quickly the depth increases, just so you know what you're dealing with - you never know if 
...
if __name__ == "__main__":
    inputs = np.loadtxt("inputs.txt")  # Read input as array
    print(np.sum(inputs[1:] > inputs[:-1]))  # Sum which elements are larger than the previous element
