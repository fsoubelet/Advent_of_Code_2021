...
So, you might know that only signal wires b and g are turned on, but that doesn't mean segments b and g are turned on: the only digit 
that uses two segments is 1, so it must mean segments c and f are meant to be on. With just that information, you still can't tell which wire (b/g) 
goes to which segment (c/f). For that, you'll need to collect more information.
...
def read_line_output_digits(line) -> List[str]:
    """Returns the strings of the output digits in a line."""
    return line.split("|")[-1].split()


# The solution is to count the number of output digits that are 1, 4, 7 or 8; which
# means counting the digits with 2, 4, 3 or 7 segments (letters) in the second part of the line.
if __name__ == "__main__":
    amount = 0
    for line in Path("inputs.txt").read_text().splitlines():
        output_digits = read_line_output_digits(line)
